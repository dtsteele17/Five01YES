-- =======================================================
-- COMPLETE FIX FOR TOURNAMENT LOADING ISSUES
-- =======================================================

-- First, let's make sure RLS is properly configured for all tournament tables
...
-- =======================================================
-- 4. Create a view for easy tournament querying
-- =======================================================
...
-- Enable RLS on the view (it inherits from tournaments)
ALTER VIEW tournament_summary SET (security_invoker = on);

-- =======================================================
-- 5. Create function to get tournament with user status
-- =======================================================
...
-- =======================================================
-- 6. Update tournament status based on timing
-- =======================================================

-- Function to automatically update tournament statuses
...
BEGIN
  -- Test the function
  SELECT get_tournaments_with_user_status() INTO v_test_result;
  
  RAISE NOTICE 'Tournament visibility setup completed successfully!';
  RAISE NOTICE '';
  RAISE NOTICE 'What this fixes:';
  RAISE NOTICE '- All authenticated users can see all tournaments';
  RAISE NOTICE '- Tournament participant counts work correctly';
  RAISE NOTICE '- User registration status is properly tracked';
  RAISE NOTICE '- RLS policies are simplified and working';
  RAISE NOTICE '';
  RAISE NOTICE 'Frontend can now use either:';
  RAISE NOTICE '1. Direct query: tournaments table with tournament_participants join';
  RAISE NOTICE '2. Function call: get_tournaments_with_user_status()';
  RAISE NOTICE '3. View query: tournament_summary view';
END $$;