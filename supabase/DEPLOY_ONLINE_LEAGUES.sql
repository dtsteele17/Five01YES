...
-- ============================================================
-- STEP 2: Extend league_members table
-- ============================================================
...
BEGIN
  ...
  IF v_member_count >= v_league.max_participants THEN RETURN json_build_object('error', 'League is full'); END IF;
  IF v_league.close_date IS NOT NULL AND now() > v_league.close_date THEN RETURN json_build_object('error', 'Registration deadline has passed'); END IF;
...
BEGIN
  ...
  IF v_member IS NULL THEN RETURN json_build_object('error', 'Not a member'); END IF;
  IF v_member.role = 'owner' THEN RETURN json_build_object('error', 'Owner cannot leave. Transfer ownership or delete the league.'); END IF;
...
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  v_league JSON; v_standings JSON; v_fixtures JSON; v_members JSON;
  v_is_member BOOLEAN; v_is_owner BOOLEAN;
BEGIN
  ...
  FROM (
    SELECT ls.user_id, (SELECT username FROM profiles WHERE id = ls.user_id) AS username,
      ls.played, ls.won, ls.lost, ls.legs_for, ls.legs_against,
      (ls.legs_for - ls.legs_against) AS leg_diff, ls.points, ls.average, ls.form
    FROM league_standings ls WHERE ls.league_id = p_league_id
  ) s;

  SELECT json_agg(row_to_json(f)) INTO v_fixtures
  FROM (
    (SELECT lf.id, lf.matchday, lf.scheduled_date, lf.scheduled_time, lf.status,
      lf.home_legs_won, lf.away_legs_won, lf.home_user_id, lf.away_user_id,
    ...
    FROM league_fixtures lf WHERE lf.league_id = p_league_id AND lf.status IN ('scheduled','live')
    ORDER BY lf.scheduled_date ASC, lf.scheduled_time ASC LIMIT 20)
    UNION ALL
    (SELECT lf.id, lf.matchday, lf.scheduled_date, lf.scheduled_time, lf.status,
      lf.home_legs_won, lf.away_legs_won, lf.home_user_id, lf.away_user_id,
      (SELECT username FROM profiles WHERE id = lf.home_user_id) AS home_username,
      (SELECT username FROM profiles WHERE id = lf.away_user_id) AS away_username
    FROM league_fixtures lf WHERE lf.league_id = p_league_id AND lf.status IN ('completed','forfeit')
    ORDER BY lf.completed_at DESC LIMIT 20)
  ) f;
  ...
  RETURN json_build_object(
    'league', v_league, 'standings', v_standings, 'fixtures', v_fixtures,
    'members', v_members, 'is_member', v_is_member, 'is_owner', v_is_owner,
...
BEGIN
  ...
  IF v_league.id IS NULL THEN RETURN json_build_object('error', 'Not authorized or league not found'); END IF;
  IF v_league.status != 'open' THEN RETURN json_build_object('error', 'League is not in open status'); END IF;

  SELECT ARRAY_AGG(user_id) INTO v_members FROM league_members WHERE league_id = p_league_id AND status = 'active';
  v_member_count := array_length(v_members, 1);
  IF v_member_count < 2 THEN RETURN json_build_object('error', 'Need at least 2 players to start'); END IF;
  ...
  FOR v_i IN 1..v_member_count LOOP
    FOR v_j IN (v_i + 1)..v_member_count LOOP
      INSERT INTO league_fixtures (league_id, home_user_id, away_user_id, matchday, scheduled_date, scheduled_time)
      VALUES (p_league_id, v_members[v_i], v_members[v_j], v_matchday, v_fixture_date, v_league.match_time);
...
CREATE OR REPLACE FUNCTION rpc_complete_league_match(p_fixture_id UUID, p_home_legs SMALLINT, p_away_legs SMALLINT)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  v_fixture league_fixtures; v_league leagues; v_home_won BOOLEAN;
BEGIN
  ...
  IF v_fixture.status = 'completed' THEN RETURN json_build_object('error', 'Match already completed'); END IF;

  SELECT * INTO v_league FROM leagues WHERE id = v_fixture.league_id;
  v_home_won := p_home_legs > p_away_legs;

  UPDATE league_fixtures SET status = 'completed', home_legs_won = p_home_legs, away_legs_won = p_away_legs, completed_at = now()
  WHERE id = p_fixture_id;

  UPDATE league_standings SET
    played = played + 1, won = won + CASE WHEN v_home_won THEN 1 ELSE 0 END,
    lost = lost + CASE WHEN v_home_won THEN 0 ELSE 1 END,
    legs_for = legs_for + p_home_legs, legs_against = legs_against + p_away_legs,
  ...
  UPDATE league_standings SET
    played = played + 1, won = won + CASE WHEN v_home_won THEN 0 ELSE 1 END,
    lost = lost + CASE WHEN v_home_won THEN 1 ELSE 0 END,
    legs_for = legs_for + p_away_legs, legs_against = legs_against + p_home_legs,
...
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF NOT EXISTS(SELECT 1 FROM leagues WHERE id = p_league_id AND owner_id = auth.uid()) THEN RETURN json_build_object('error', 'Not authorized'); END IF;
  IF p_user_id = auth.uid() THEN RETURN json_build_object('error', 'Cannot kick yourself'); END IF;
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM leagues WHERE id = p_league_id AND owner_id = auth.uid()) THEN RETURN json_build_object('error', 'Not authorized'); END IF;
  INSERT INTO league_warnings (league_id, user_id, admin_id, reason) VALUES (p_league_id, p_user_id, auth.uid(), p_reason);
  RETURN json_build_object('success', true);
END; $$;
...
BEGIN
  SELECT * INTO v_fixture FROM league_fixtures WHERE id = p_fixture_id;
  IF NOT EXISTS(SELECT 1 FROM leagues WHERE id = v_fixture.league_id AND owner_id = auth.uid()) THEN RETURN json_build_object('error', 'Not authorized'); END IF;
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM leagues WHERE id = p_league_id AND owner_id = auth.uid()) THEN RETURN json_build_object('error', 'Not authorized'); END IF;
  UPDATE leagues SET name = COALESCE(p_name, name), description = COALESCE(p_description, description) WHERE id = p_league_id;
  RETURN json_build_object('success', true);
END; $$;
