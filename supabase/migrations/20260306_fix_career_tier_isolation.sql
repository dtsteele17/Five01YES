-- ============================================================
-- Fix Career Tier Isolation - Prevent Wrong Tier Events
-- Fixes Local Circuit Cup appearing in Pub Leagues
-- ============================================================
...
WHERE career_id IN (
  ...
  WHERE cp.tier != (
    ...
    WHERE event_name = ce.event_name 
    LIMIT 1
  )
);
...
BEGIN
  ...
  FROM career_events ce
  JOIN career_schedule_templates cst ON cst.id = ce.template_id
  WHERE ce.career_id = p_career_id 
    AND ce.status = 'pending'
    AND cst.tier = v_career.tier  -- ENSURE TIER MATCH
  ...
  IF v_event.id IS NULL THEN
    ...
    WITH next_template AS (
      ...
      WHERE tier = v_career.tier
        AND sequence_no > COALESCE((
          ...
          JOIN career_schedule_templates cst2 ON cst2.id = ce2.template_id
          WHERE ce2.career_id = p_career_id AND cst2.tier = v_career.tier
        ), 0)
    ...
    INSERT INTO career_events (
      ...
      event_name, format_legs, bracket_size, status, day, week
    )
    SELECT 
      p_career_id, nt.id, v_career.season, nt.sequence_no, nt.event_type,
      nt.event_name, nt.format_legs, nt.bracket_size, 'pending',
      CASE WHEN nt.day_based THEN nt.metadata->>'day' ELSE NULL END::INT,
      CASE WHEN NOT COALESCE(nt.day_based, FALSE) THEN v_career.week + 1 ELSE NULL END
...
CREATE OR REPLACE FUNCTION rpc_get_career_home_safe(
  p_career_id UUID
)
RETURNS JSON
LANGUAGE plpgsql
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      WHERE ce.career_id = career_record.id 
        AND ce.status = 'pending'
        AND cst.tier = career_record.tier
    ) THEN
      ...
      IF template_record.id IS NOT NULL THEN
        INSERT INTO career_events (
          ...
          CASE WHEN NOT COALESCE(template_record.day_based, FALSE) THEN career_record.week ELSE NULL END
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed career tier isolation: Removed wrong-tier events, ensured proper tier boundaries in event progression';
END $$;