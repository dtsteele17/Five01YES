/*
  # BOLT FIX: Complete Turn Switching Fix

  This migration fixes ALL turn switching issues for Bolt deployment:
  1. Fixes rpc_quick_match_submit_visit_v3 to use UUID for current_turn
  2. Fixes rpc_quick_match_submit_visit_v2 as backup
  3. Handles NULL current_turn values
  4. Ensures proper turn switching with atomic updates

  Run this in Bolt SQL Editor to fix everything at once.
*/

-- ============================================================================
-- PART 1: Fix rpc_quick_match_submit_visit_v3 (MAIN FUNCTION)
-- ============================================================================
...
COMMENT ON FUNCTION public.rpc_quick_match_submit_visit_v3 IS 'V3: Fixed to use UUID for current_turn. Stores visits in quick_match_visits table with full dart details, darts_thrown, and darts_at_double. Handles manual bust, automatic bust detection, double-out validation (only when remaining <= 50), and match progression.';

-- ============================================================================
-- PART 2: Fix rpc_quick_match_submit_visit_v2 (BACKUP FUNCTION)
-- ============================================================================
...
BEGIN
  ...
  -- Update remaining and switch turn atomically
  UPDATE match_rooms
  SET player1_remaining = CASE WHEN v_is_player1 THEN v_new_remaining ELSE v_room.player1_remaining END,
      player2_remaining = CASE WHEN v_is_player1 THEN v_room.player2_remaining ELSE v_new_remaining END,
      current_turn = v_other_player_id  -- Switch to opponent's UUID
  ...
  -- Return response (v2 format for compatibility)
  RETURN jsonb_build_object(
    'ok', TRUE,
    'remaining_after', v_new_remaining,
    'is_bust', v_is_bust,
...
COMMENT ON FUNCTION public.rpc_quick_match_submit_visit_v2 IS 'V2: Fixed to use UUID for current_turn. DEPRECATED - Use rpc_quick_match_submit_visit_v3 instead.';

-- ============================================================================
-- PART 3: Fix any existing rooms with NULL current_turn
-- ============================================================================
