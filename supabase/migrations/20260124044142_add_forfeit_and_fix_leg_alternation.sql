/*
  # Add Forfeit Function and Fix Leg Starter Alternation

  1. New Functions
    - `forfeit_quick_match(p_room_id UUID)`
      - Allows a player to forfeit their match
      - Sets the opponent as winner
      - Creates forfeit event
      - Marks match as finished

  2. Bug Fixes
    - Update `submit_quick_match_throw` to alternate starting player each leg
    - Previously always started with player1
    - Now alternates based on leg number (odd legs: player1, even legs: player2)

  3. Security
    - Only authenticated players in the match can forfeit
    - Atomic updates to prevent race conditions
*/

-- Create forfeit function
...
-- Fix submit_quick_match_throw to alternate starting player
