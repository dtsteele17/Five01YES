-- ============================================
-- DARTBOT STATS COMPLETE FIX
-- ============================================
-- This migration ensures dartbot match stats are recorded correctly
-- including the winning throw, checkout percentage, and highest checkout.
-- It also ensures opponent (bot) stats are saved to metadata.
...
-- ============================================
-- STEP 3: Create updated RPC function
-- ============================================
...
BEGIN
  ...
  v_total_checkouts := CASE WHEN p_player_checkout_pct > 0 THEN p_player_legs_won ELSE 0 END;
  ...
  ELSE
    ...
    UPDATE player_stats SET
      ...
      checkout_percentage = CASE 
        WHEN (checkout_attempts + v_checkout_attempts) > 0 
        THEN ROUND(((total_checkouts + v_total_checkouts)::DECIMAL / (checkout_attempts + v_checkout_attempts)) * 100, 2)
        ELSE 0 
      END,
...
-- ============================================
-- STEP 4: Create or update index for metadata queries
-- ============================================
-- This helps with queries that filter by match_format
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE indexname = 'idx_match_history_user_format'
  ) THEN
    CREATE INDEX idx_match_history_user_format ON match_history(user_id, match_format);
  END IF;
END $$;

-- ============================================
-- STEP 5: Update RLS policies to allow reading metadata
-- ============================================
DO $$
BEGIN
  -- Ensure the metadata column is accessible
  -- The existing policies should cover this, but let's make sure
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'match_history' AND policyname = 'Users can view their own match history'
  ) THEN
    -- Policy exists, no need to change
    NULL;
  END IF;
END $$;

SELECT 'Dartbot stats fix migration completed successfully!' as status;
