-- ============================================================
-- FIX: Season end - proper promotion and new season logic
-- 
-- Top 2 → promoted to next tier, new season with new tier's schedule
-- Not top 2 → stay in same tier, new season, top 2 opponents replaced
-- ============================================================
  ...
  IF v_event.event_type = 'league' AND v_career.tier >= 2 THEN
    ...
    IF v_completed_league_count = 4 AND v_career.tier = 2 THEN
      IF NOT EXISTS (
        ...
        INSERT INTO career_events (
          ...
          3, 16, 'pending', COALESCE(v_event.day, v_career.day) + 3
        ...
        INSERT INTO career_milestones (
          ...
          p_career_id, 'tournament_invite', v_tournament_name,
          'You''ve been invited to ' || v_tournament_name || '! A 16-player knockout at the local.',
    ...
    BEGIN
      ...
      IF v_player_played >= v_total_opponents THEN
        ...
        -- Fill up AI standings so everyone has equal games
        UPDATE career_league_standings SET
        ...
        -- Rank all opponents by final standings
        ...
        IF v_player_rank <= 2 THEN
          ...
          -- Create new season events from templates for new tier
        ...
        ELSE
          -- ========== NOT PROMOTED — STAY IN SAME TIER ==========
          ...
          -- Seed new season events (same tier)
          INSERT INTO career_events (career_id, template_id, season, sequence_no, event_type, event_name, format_legs, bracket_size, day)
          SELECT p_career_id, t.id, v_new_season, t.sequence_no, t.event_type, t.event_name, t.format_legs, t.bracket_size,
            v_new_day + t.sequence_no * 6
          FROM career_schedule_templates t WHERE t.tier = v_new_tier ORDER BY t.sequence_no;
