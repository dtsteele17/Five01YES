/*
  # Recreate RPC function to edit Quick Match visit scores
  
  1. Function Purpose
    - Allows players to edit a visit score in a Quick Match
    - Recalculates remaining_after for the edited visit and all subsequent visits
    - Updates room state (player remaining scores) if the edit affects the current leg
  
  2. Parameters
    - p_event_id: UUID of the match_event to edit
    - p_new_score: New score value (0-180)
  
  3. Logic
    - Validates the event exists and is a visit event
    - Recalculates remaining_after based on previous visit
    - Updates all subsequent visits in the same leg
    - Updates room player remaining scores for current leg
    - Handles checkout detection (remaining_after = 0)
  
  4. Security
    - RLS policies on match_events and match_rooms will control access
*/
