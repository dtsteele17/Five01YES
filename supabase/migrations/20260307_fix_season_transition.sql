-- ============================================================
-- FIX: Season transition flow
-- 
-- PROBLEM: rpc_career_complete_match instantly creates next season
-- after the last match, so the user never sees "Season Complete".
--
-- FIX: After last match, DON'T create new season. Just mark the
-- season as complete. A separate RPC handles "Next Season" click.
-- ============================================================

-- New RPC: Advance to next season (called when user clicks "Next Season")
...
BEGIN
  ...
  -- Check that current season is actually complete
  ...
  -- Calculate player rank in current season
  ...
  ELSE
    ...
    -- Generate 2 new opponents to replace promoted ones
...
-- Now update rpc_career_complete_match to NOT create next season
-- Just fill up AI standings and record milestones
...
BEGIN
  ...
  IF v_event.event_type = 'league' AND v_career.tier >= 2 THEN
    ...
    -- AI vs AI simulation with correct leg math
    <<ai_sim>>
    DECLARE
      v_ai_ids UUID[];
      v_ai_count INT;
    ...
    BEGIN
      ...
      WHILE v_idx + 1 <= v_ai_count LOOP
        ...
        UPDATE career_league_standings SET
          ...
          points = points + 2
        WHERE career_id = p_career_id AND season = v_career.season AND tier = v_career.tier
          AND opponent_id = CASE WHEN v_home_won THEN v_ai_ids[v_idx] ELSE v_ai_ids[v_idx + 1] END;

        UPDATE career_league_standings SET
          played = played + 1, lost = lost + 1,
          legs_for = legs_for + v_l_legs, legs_against = legs_against + v_w_legs
        WHERE career_id = p_career_id AND season = v_career.season AND tier = v_career.tier
          AND opponent_id = CASE WHEN v_home_won THEN v_ai_ids[v_idx + 1] ELSE v_ai_ids[v_idx] END;
    ...
    IF v_completed_league_count = 4 AND v_career.tier = 2 THEN
      IF NOT EXISTS (
        ...
        INSERT INTO career_events (
          ...
          3, 16, 'pending_invite', COALESCE(v_event.day, v_career.day) + 3
        ) RETURNING id INTO v_tournament_event_id;
      END IF;
    END IF;

    -- Season complete: just fill up AI standings, DON'T create next season
    -- The user will click "Next Season" which calls rpc_career_advance_to_next_season
    <<season_check>>
    DECLARE
      v_player_played SMALLINT;
      v_player_rank SMALLINT;
      v_total_opps SMALLINT;
    BEGIN
      SELECT played INTO v_player_played FROM career_league_standings
      WHERE career_id = p_career_id AND season = v_career.season AND tier = v_career.tier AND is_player = TRUE;

      SELECT COUNT(*)::SMALLINT INTO v_total_opps FROM career_league_standings
      WHERE career_id = p_career_id AND season = v_career.season AND tier = v_career.tier AND is_player = FALSE;

      IF v_player_played >= v_total_opps THEN
        -- Fill up AI standings
        UPDATE career_league_standings SET
          played = v_total_opps,
          won = LEAST(won, v_total_opps),
          lost = v_total_opps - LEAST(won, v_total_opps)
        WHERE career_id = p_career_id AND season = v_career.season AND tier = v_career.tier
          AND is_player = FALSE AND played < v_total_opps;

        -- Calculate rank for return value
        SELECT COUNT(*)::SMALLINT + 1 INTO v_player_rank FROM career_league_standings
        WHERE career_id = p_career_id AND season = v_career.season AND tier = v_career.tier
          AND is_player = FALSE
          AND (points > (SELECT points FROM career_league_standings WHERE career_id = p_career_id AND season = v_career.season AND tier = v_career.tier AND is_player = TRUE)
        ...
        v_is_promotion := v_player_rank <= 2;
      END IF;
    END season_check;
  END IF;
