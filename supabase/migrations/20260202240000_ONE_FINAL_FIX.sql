/*
  # ONE FINAL FIX - Make it work NOW
  
  This fixes:
  1. Creates ready_up_tournament_match RPC (fixes 404)
  2. Uses correct table structure
  3. Works for both players
*/

-- ============================================================
-- 1. CREATE/FIX ready_up_tournament_match RPC
-- ============================================================
...
BEGIN
  ...
  -- INSERT ready status - THIS MUST WORK FOR BOTH PLAYERS
  ...
  IF v_ready_count >= 2 AND v_match.match_room_id IS NULL THEN
    ...
    IF FOUND THEN
      ...
      -- Check if online_matches table exists, if not use match_rooms
      BEGIN
        INSERT INTO online_matches (
          ...
          CASE v_best_of WHEN 1 THEN 'best-of-1' WHEN 3 THEN 'best-of-3' WHEN 5 THEN 'best-of-5' WHEN 7 THEN 'best-of-7' ELSE 'best-of-3' END,
          true, 'in_progress', 1, v_tournament.game_mode, v_tournament.game_mode, v_match.player1_id
        ) RETURNING id INTO v_match_room_id;
      EXCEPTION WHEN undefined_table THEN
        -- Fallback to match_rooms if online_matches doesn't exist
        INSERT INTO match_rooms (
          player1_id, player2_id, game_mode, status, legs_to_win,
          player1_remaining, player2_remaining, current_turn
        ) VALUES (
          v_match.player1_id, v_match.player2_id, v_tournament.game_mode, 'active',
      ...
      RETURN jsonb_build_object('ready_count', v_ready_count, 'status', 'in_game', 'match_room_id', v_match_room_id);
    END IF;
  END IF;

  RETURN jsonb_build_object('ready_count', v_ready_count, 'status', 'ready', 'match_room_id', NULL);
END;
$$;
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_ready_count FROM tournament_match_ready WHERE match_id = NEW.match_id;
