/*
  # Update Request Rematch RPC with Synchronized Start

  1. Changes
    - Update request_rematch function to set start_at timestamp
    - start_at is set 2 seconds in the future when new room is created
    - Ensures both clients redirect at the same time

  2. Notes
    - Only sets start_at when both players are ready
    - Returns start_at in response for immediate handling
    - Prevents race condition in redirects
*/

-- Drop and recreate the function with start_at support
...
BEGIN
  ...
  IF v_ready_count = 2 AND v_rematch.new_room_id IS NULL THEN
    -- Set start time 2 seconds in the future for synchronized start
    v_start_at := now() + interval '2 seconds';
    ...
    -- Update rematch record with new room and start time
    UPDATE match_rematches
    SET 
    ...
    VALUES 
      ...
      (v_old_room.player2_id, 'match_invite', 'Rematch Ready!', 'Both players ready - match starting',
       jsonb_build_object('match_id', v_new_room_id, 'link', '/app/play/quick-match/match/' || v_new_room_id));
  ELSE
    v_new_room_id := v_rematch.new_room_id;
    v_start_at := v_rematch.start_at;
  END IF;

  -- Return ready count, new room id, and start time
