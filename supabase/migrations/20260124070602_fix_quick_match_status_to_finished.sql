/*
  # Fix Quick Match Status to use 'finished'

  1. Changes
    - Update `submit_quick_match_throw` to use 'finished' status when match is won
    - Ensures consistency with match_rooms status constraint
    - Aligns with standard match completion status

  2. Notes
    - Status values allowed: 'open', 'in_progress', 'active', 'finished', 'forfeited'
    - During gameplay: status = 'active'
    - When match is won: status = 'finished'
*/
...
BEGIN
  ...
  IF v_is_checkout THEN
    ...
    END IF;
  ELSE
    -- Update remaining and switch turn (match stays active)
    UPDATE public.match_rooms
    SET
      player1_remaining = CASE WHEN v_is_player1 THEN v_new_remaining ELSE player1_remaining END,
      player2_remaining = CASE WHEN v_is_player1 THEN player2_remaining ELSE v_new_remaining END,
