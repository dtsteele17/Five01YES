-- ============================================================
-- Ensure Proper Tier 2 Structure: 4 League + Tournament + 3 League (FIXED)
-- ============================================================
...
DECLARE
    ...
    v_sequence_no INT;  -- Renamed to avoid ambiguity
BEGIN
        ...
        IF league_count != 7 OR tournament_count = 0 THEN
            ...
            FOR i IN 1..4 LOOP
                ...
                VALUES (
                    ...
                    v_sequence_no,  -- Use variable explicitly
                    'league',
                    'Weekend League Night — Matchday ' || i,
                    3,
                    NULL,
                    CASE WHEN v_sequence_no <= career_record.week THEN 'completed' ELSE 'pending' END
                );
...
BEGIN
  ...
  FOR event_record IN
    SELECT ce.sequence_no, ce.event_type, ce.event_name, ce.status, 
  ...
  RETURN json_build_object(
    ...
    'next_event', json_build_object(
      'type', COALESCE(v_next_event.event_type, 'none'),
      'name', COALESCE(v_next_event.event_name, 'none'),
      'sequence', COALESCE(v_next_event.sequence_no, 0)
    ),
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed variable ambiguity in Tier 2 structure migration';
END $$;