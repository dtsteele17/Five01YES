-- ============================================================================
-- FIX: Quick Match Stats Recording
-- This ensures stats are properly saved when a quick match ends
-- ============================================================================

-- 1. Ensure match_history has all required columns
...
-- 6. Create or replace the main function for recording quick match stats
...
BEGIN
  -- Record winner stats
  SELECT id INTO v_winner_history_id
  FROM fn_update_player_match_stats(
    p_room_id,
    p_winner_id,
    p_loser_id,
    'win',
  ...
  -- Record loser stats
  SELECT id INTO v_loser_history_id
  FROM fn_update_player_match_stats(
    p_room_id,
    p_loser_id,
    p_winner_id,
...
-- 7. Ensure the core fn_update_player_match_stats function exists and works
...
-- 10. Verify
SELECT 'Quick match stats recording fixed' as status;
