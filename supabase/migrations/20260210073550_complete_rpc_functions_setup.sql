...
DROP POLICY IF EXISTS "Users can view their invites" ON private_match_invites;
CREATE POLICY "Users can view their invites"
  ON private_match_invites FOR SELECT
  TO authenticated
  USING (auth.uid() = from_user_id OR auth.uid() = to_user_id);

DROP POLICY IF EXISTS "Users can create invites" ON private_match_invites;
CREATE POLICY "Users can create invites"
  ON private_match_invites FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = from_user_id);

DROP POLICY IF EXISTS "Users can update their invites" ON private_match_invites;
