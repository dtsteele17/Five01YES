-- Final fixes for checkout validation and leg tracking
-- This ensures the RPC function works correctly for winning visits
...
-- Create a clean function with proper checkout handling
...
BEGIN
  ...
  -- Count darts (handle empty array)
  ...
  -- Update room state based on outcome
...
-- Ensure leg columns exist and have proper defaults
ALTER TABLE public.match_rooms 
ALTER COLUMN player1_legs SET DEFAULT 0,
ALTER COLUMN player2_legs SET DEFAULT 0;

-- Update any NULL leg values to 0
UPDATE public.match_rooms 
SET player1_legs = 0 
WHERE player1_legs IS NULL;

UPDATE public.match_rooms 
SET player2_legs = 0 
WHERE player2_legs IS NULL;
