-- ============================================================================
-- FIX: Quick Match Opponent Stats Recording
-- 
-- The fn_update_player_match_stats function was not saving opponent stats.
-- This fixes it to save both player and opponent stats.
-- ============================================================================

-- 1. Ensure match_history has all opponent stats columns
...
-- 2. Drop the broken function and recreate with opponent stats support
DROP FUNCTION IF EXISTS fn_update_player_match_stats(UUID, UUID, UUID, TEXT, INTEGER, INTEGER, INTEGER);

-- 3. Create the FIXED function that saves BOTH player and opponent stats
...
-- 4. Drop and recreate the wrapper function (return type changed from VOID to JSONB)
...
-- 6. Create a helpful view to verify opponent stats are being saved
CREATE OR REPLACE VIEW v_match_history_verify AS
SELECT 
  mh.id,
  mh.room_id,
  mh.user_id,
  ...
  mh.legs_won,
  mh.legs_lost,
  mh.three_dart_avg as user_avg,
  mh.visits_180 as user_180s,
  mh.opponent_three_dart_avg as opponent_avg,
  mh.opponent_visits_180 as opponent_180s,
  CASE 
    WHEN mh.opponent_three_dart_avg > 0 THEN 'YES'
    ELSE 'NO'
  END as has_opponent_stats,
  mh.played_at
FROM match_history mh
ORDER BY mh.played_at DESC
LIMIT 100;

GRANT SELECT ON v_match_history_verify TO authenticated;

-- 7. Verify the fix
SELECT 'Quick match opponent stats recording FIXED!' as status;
