/*
  # Update ready_up_tournament_match to Return JSON Response

  1. Changes
    - Update ready_up_tournament_match to return JSON with ready_count, status, and match_room_id
    - Returns { ready_count: N, status: 'ready' or 'in_game', match_room_id: uuid or null }
    - Allows frontend to update UI with ready count and navigate when both ready

  2. Purpose
    - Provide complete match state information in a single response
    - Allow UI to update ready count immediately after ready-up
    - Simplify navigation flow with status and room_id in response
*/
