-- ============================================================================
-- FIX: First 9 Dart Average Calculation
-- ============================================================================
-- The first 9 average was being calculated incorrectly - it was using total
-- darts thrown instead of just the first 9 darts of each leg.
-- 
-- The fix adds columns to track cumulative first 9 stats and updates the
-- calculation to properly weight the average across all matches.

-- Add columns to track cumulative first 9 stats
ALTER TABLE public.player_stats 
ADD COLUMN IF NOT EXISTS first9_total_score INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS first9_total_darts INTEGER DEFAULT 0;

-- Update the main stats function to properly calculate first 9 average
...
DECLARE
  ...
  v_new_first9_score INTEGER;
  v_new_first9_darts INTEGER;
  v_new_first9_avg DECIMAL(5,2);
...
BEGIN
  ...
  LOOP
    ...
    -- First 9 calculation - ONLY count first 3 visits (9 darts)
  ...
  -- FIX: Calculate first 9 average using ONLY first 9 darts
  ...
  IF v_existing IS NULL THEN
    ...
    INSERT INTO public.player_stats (
      ...
      first9_total_score, first9_total_darts,
      ...
      v_match_first9_score, v_match_first9_darts,
  ...
  ELSE
    ...
    -- FIX: Accumulate first 9 stats separately
    v_new_first9_score := COALESCE(v_existing.first9_total_score, 0) + v_match_first9_score;
    v_new_first9_darts := COALESCE(v_existing.first9_total_darts, 0) + v_match_first9_darts;
    ...
    -- FIX: Calculate first 9 average using only first 9 darts
    IF v_new_first9_darts > 0 THEN
      v_new_first9_avg := ROUND(((v_new_first9_score::DECIMAL / v_new_first9_darts) * 3)::DECIMAL, 2);
    END IF;
    ...
    UPDATE public.player_stats SET
      ...
      -- FIX: Store cumulative first 9 stats and calculated average
      first9_total_score = v_new_first9_score,
      first9_total_darts = v_new_first9_darts,
      overall_first9_avg = v_new_first9_avg,
  ...
  RETURN jsonb_build_object(
    ...
    'match_first9_avg', v_match_first9_avg,
...
-- Reset existing first 9 averages to 0 so they can be recalculated correctly
-- This will force the stats to be recalculated on next match
UPDATE public.player_stats SET overall_first9_avg = 0 WHERE overall_first9_avg > 200;

SELECT 'First 9 average calculation fixed!' as status;
