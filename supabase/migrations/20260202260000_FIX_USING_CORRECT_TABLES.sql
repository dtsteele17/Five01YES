/*
  # FIX USING CORRECT TABLES
  
  Issues found:
  1. online_matches doesn't exist - use match_rooms instead
  2. tournament_match_ready.user_id references profiles.user_id
  3. tournament_matches.player1_id/player2_id are auth.users.id
  
  This fixes everything using the ACTUAL table structure.
*/

-- ============================================================
-- 1. FIX ready_up_tournament_match - Use match_rooms (not online_matches)
-- ============================================================
...
BEGIN
  ...
  -- Get profile ID (tournament_match_ready.user_id is profiles.user_id)
  SELECT id INTO v_profile_id FROM profiles WHERE user_id = v_user_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('error', 'Profile not found');
  END IF;

  -- INSERT ready status using profile.user_id (not auth.users.id)
...
-- ============================================================
-- 2. FIX TRIGGER - Use match_rooms (not online_matches)
-- ============================================================
