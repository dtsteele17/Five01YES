-- Fix the running average calculation in fn_update_player_match_stats
-- The issue was that total_matches was already incremented in the UPDATE clause,
-- so we need to use (total_matches - 1) to get the previous value for the calculation
...
BEGIN
  ...
  -- FIX: Use proper weighted average calculation
  -- New Average = ((Old Average * Old Match Count) + New Match Average) / New Match Count
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    -- FIX: Calculate weighted average correctly using (total_matches) as the OLD count
    -- Since total_matches is already incremented by +1 above, we use it directly
    overall_3dart_avg = ((public.player_stats.overall_3dart_avg * (public.player_stats.total_matches - 1)) + v_three_dart_avg) / public.player_stats.total_matches,
    overall_first9_avg = ((public.player_stats.overall_first9_avg * (public.player_stats.total_matches - 1)) + v_first9_avg) / public.player_stats.total_matches,
    -- Max/highest values
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    -- FIX: Calculate weighted average correctly
    overall_3dart_avg = ((public.player_quick_match_stats.overall_3dart_avg * (public.player_quick_match_stats.total_matches - 1)) + v_three_dart_avg) / public.player_quick_match_stats.total_matches,
    overall_first9_avg = ((public.player_quick_match_stats.overall_first9_avg * (public.player_quick_match_stats.total_matches - 1)) + v_first9_avg) / public.player_quick_match_stats.total_matches,
...
COMMENT ON FUNCTION public.fn_update_player_match_stats IS 'Updates player stats with proper weighted average calculation';
