-- ============================================================
-- EMERGENCY FIX: Use ACTUAL Table Schema (corrected)
-- Fix Continue button and opponent consistency with correct columns
-- ============================================================

-- 1. FIRST: Properly clean Local Circuit Cup from Tier 2+
...
BEGIN
    -- Find active events without pending matches
    FOR event_record IN 
    ...
    LOOP
        ...
        -- Create the missing match using ACTUAL table columns
...
-- 3. Update the home RPC to ensure opponent consistency (using correct columns)
...
DO $$
BEGIN
  RAISE NOTICE 'Emergency fix applied with CORRECT schema: career_matches uses result (not status), format_legs (not best_of)';
END $$;