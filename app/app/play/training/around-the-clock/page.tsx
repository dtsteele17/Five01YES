  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4">
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        >
          <Card className="bg-gradient-to-br from-blue-600/20 via-purple-600/20 to-pink-600/20 border-blue-500/30 p-4 sm:p-8 text-center backdrop-blur-sm relative overflow-hidden">
            {/* Animated background effect */}
            <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-pink-500/10 animate-pulse" />
            
            <div className="relative z-10">
              <div className="flex items-center justify-center gap-2 mb-2 sm:mb-4">
                <Crosshair className="h-4 w-4 sm:h-6 sm:w-6 text-blue-400" />
                <span className="text-sm sm:text-lg font-medium text-blue-300">Current Target</span>
              </div>
              ...
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="text-5xl sm:text-7xl font-black text-white drop-shadow-lg"
                >
                  {currentTargetLabel}
                </motion.div>
              </AnimatePresence>
              ...
              {state.completedTargets.length > 0 && (
                <div className="mt-3 sm:mt-6 flex flex-wrap justify-center gap-1.5 sm:gap-2 max-w-md mx-auto">
                  {state.completedTargets.slice(-8).map((target, idx) => (
                    <motion.div
                      key={idx}
                      initial={{ scale: 0, rotate: -180 }}
                      animate={{ scale: 1, rotate: 0 }}
                      className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center text-white font-bold text-xs sm:text-sm shadow-lg"
                    >
                      {getTargetLabel(target)}
                    </motion.div>
                  ))}
                  {state.completedTargets.length > 8 && (
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-700 flex items-center justify-center text-slate-400 text-xs">
                      +{state.completedTargets.length - 8}
                    </div>
                  )}
                </div>
        ...
        >
          <Card className="bg-slate-800/50 border-slate-700/50 p-2 sm:p-4 text-center backdrop-blur-sm">
            <Zap className="h-4 w-4 text-yellow-400 mx-auto mb-1" />
            <div className="text-lg sm:text-2xl font-black text-white">{state.totalThrows}</div>
            <div className="text-[10px] sm:text-xs text-slate-400 font-medium uppercase">Throws</div>
          </Card>
          <Card className="bg-slate-800/50 border-slate-700/50 p-2 sm:p-4 text-center backdrop-blur-sm">
            <CheckCircle2 className="h-4 w-4 text-emerald-400 mx-auto mb-1" />
            <div className="text-lg sm:text-2xl font-black text-white">{state.hits}</div>
            <div className="text-[10px] sm:text-xs text-slate-400 font-medium uppercase">Hits</div>
          </Card>
          <Card className="bg-slate-800/50 border-slate-700/50 p-2 sm:p-4 text-center backdrop-blur-sm">
            <Target className="h-4 w-4 text-blue-400 mx-auto mb-1" />
            <div className="text-lg sm:text-2xl font-black text-white">{accuracy}%</div>
            <div className="text-[10px] sm:text-xs text-slate-400 font-medium uppercase">Accuracy</div>
          </Card>
          <Card className="bg-slate-800/50 border-slate-700/50 p-2 sm:p-4 text-center backdrop-blur-sm">
            <Clock className="h-4 w-4 text-purple-400 mx-auto mb-1" />
            <div className="text-lg sm:text-2xl font-black text-white">{formatTime(elapsedTime)}</div>
            <div className="text-[10px] sm:text-xs text-slate-400 font-medium uppercase">Time</div>
          </Card>
        </motion.div>
      ...
      <Dialog open={state.isComplete} onOpenChange={() => {}}>
        <DialogContent className="bg-gradient-to-br from-slate-800 to-slate-900 border-slate-700 max-w-sm max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center justify-center gap-2 text-2xl text-white">
              <motion.div
          ...
          >
            ...
            {/* Time Display */}
            <div className="text-center py-3 bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-pink-500/10 rounded-xl border border-white/10">
              <div className="text-2xl sm:text-4xl font-black text-white mb-1">
                {formatTime(elapsedTime)}
              </div>
              <div className="text-slate-400 text-sm font-medium">Completion Time</div>
            </div>

            {/* Stats Grid — always 2x2 */}
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-slate-700/50 p-3 rounded-xl text-center border border-slate-600/50">
                <div className="text-2xl font-black text-blue-400">{state.totalThrows}</div>
                <div className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Throws</div>
              </div>
              <div className="bg-slate-700/50 p-3 rounded-xl text-center border border-slate-600/50">
                <div className="text-2xl font-black text-emerald-400">{state.hits}</div>
                <div className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Hits</div>
              </div>
              <div className="bg-slate-700/50 p-3 rounded-xl text-center border border-slate-600/50">
              ...
              <div className="bg-slate-700/50 p-3 rounded-xl text-center border border-slate-600/50">
                <div className="text-2xl font-black text-red-400">{state.misses}</div>
                <div className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Misses</div>
              </div>
            </div>
          </motion.div>
          ...
          <DialogFooter className="gap-2 mt-4">
            <Button
              onClick={handleRetry}
              className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold py-3 sm:py-4 text-base shadow-lg shadow-blue-500/25"
            >
            ...
            <Button
              onClick={handleReturn}
              variant="outline"
              className="flex-1 border-slate-600 text-slate-300 hover:bg-slate-800 hover:text-white font-bold py-3 sm:py-4"
            >
              <ArrowLeft className="mr-2 h-4 w-4" />
              Exit
            </Button>
