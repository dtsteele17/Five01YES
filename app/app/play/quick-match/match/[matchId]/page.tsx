...
export default function QuickMatchRoomPage() {
  ...
  // Handle live video display switching based on whose turn it is
  useEffect(() => {
    console.log('[TURN CHANGE] ===== VIDEO DISPLAY EFFECT RUNNING =====');
    console.log('[TURN CHANGE] This effect ONLY updates video display, NO cleanup should occur');

    if (!liveVideoRef.current) {
      console.log('[TURN CHANGE] No video ref, skipping');
      return;
    }

    const isMyTurn = room?.current_turn === currentUserId;
    const liveStreamToShow = isMyTurn ? localStream : remoteStream;

    const turnLabel = isMyTurn ? 'me' : 'opponent';
    const hasStream = !!liveStreamToShow;
    console.log(`[VIDEO DISPLAY] turn: ${turnLabel} stream: ${hasStream}`);
    console.log('[VIDEO DISPLAY] localStream exists:', !!localStream);
    console.log('[VIDEO DISPLAY] remoteStream exists:', !!remoteStream);
    console.log('[VIDEO DISPLAY] PC exists:', !!peerConnectionRef.current);
    console.log('[VIDEO DISPLAY] PC state:', peerConnectionRef.current?.connectionState);

    // Extra diagnostics for opponent's turn
    if (!isMyTurn) {
      console.log('👤 [OPPONENT TURN] ========== OPPONENT\'S TURN DIAGNOSTICS ==========');
      console.log('👤 [OPPONENT TURN] remoteStream:', remoteStream ? 'EXISTS' : 'NULL');
      if (remoteStream) {
        console.log('👤 [OPPONENT TURN] remoteStream.id:', remoteStream.id);
        console.log('👤 [OPPONENT TURN] remoteStream.active:', remoteStream.active);
        console.log('👤 [OPPONENT TURN] remoteStream tracks:', remoteStream.getTracks().map(t => ({
          kind: t.kind,
          enabled: t.enabled,
      ...
      console.log('👤 [OPPONENT TURN] PC connectionState:', peerConnectionRef.current?.connectionState);
      console.log('👤 [OPPONENT TURN] PC iceConnectionState:', peerConnectionRef.current?.iceConnectionState);
      console.log('👤 [OPPONENT TURN] callStatus:', callStatus);
    ...
    console.log('[TURN CHANGE] Video display updated, NO cleanup performed');
    console.log('[TURN CHANGE] ==========================================');
  }, [room?.current_turn, currentUserId, localStream, remoteStream]);

  // WebRTC setup with perfect negotiation and stable transceivers
  useEffect(() => {
    const setupWebRTC = async () => {
      if (!room || !currentUserId || !iceServers) {
        console.log('[WEBRTC] Waiting for conditions:', {
          hasRoom: !!room,
      ...
      if (!opponentId) {
        console.log('[WEBRTC] No opponent yet');
        return;
      }

      // If peer connection already exists, don't recreate
      if (peerConnectionRef.current) {
        console.log('[WEBRTC] Peer connection already exists, state:', peerConnectionRef.current.connectionState);
        return;
      }

      // Deterministic polite/impolite role for perfect negotiation
      // Polite peer (larger UUID) will rollback on collision
      const isPolite = currentUserId > opponentId;
      ...
      try {
        ...
        // CRITICAL: Add transceivers BEFORE any offer/answer to prevent m-line mismatch
        // Both peers add transceivers in same order with sendrecv direction
        console.log('[WEBRTC] Adding transceivers BEFORE any negotiation...');
        ...
        console.log('   - Video: sendrecv');
        console.log('   - Audio: sendrecv');

        // ICE candidate handler
        pc.onicecandidate = (event) => {
          if (event.candidate) {
            console.log('🧊 [WEBRTC ICE] Sending ICE candidate');
        ...
        pc.oniceconnectionstatechange = () => {
          console.log('🔌 [WEBRTC ICE] iceConnectionState:', pc.iceConnectionState);

          if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') {
            console.log('✅ [WEBRTC ICE] ICE connection established');
            setCallStatus('connected');
          } else if (pc.iceConnectionState === 'failed') {
            console.error('❌ [WEBRTC ICE] ICE connection failed');
            setCallStatus('idle');
            toast.error('Connection failed');
          } else if (pc.iceConnectionState === 'disconnected') {
            console.warn('⚠️ [WEBRTC ICE] ICE disconnected');
          }
        };
        ...
        pc.onnegotiationneeded = async () => {
          try {
            ...
            if (makingOfferRef.current) {
              console.log('⚠️ [NEGOTIATION] Already making offer, skipping');
              return;
            }
            ...
            console.log('📤 [NEGOTIATION] Sending offer');
        ...
        pc.onconnectionstatechange = () => {
          console.log('🌐 [WEBRTC CONNECTION] connectionState:', pc.connectionState);

          if (pc.connectionState === 'connected') {
            console.log('✅ [WEBRTC CONNECTION] Peer connection established');
            setCallStatus('connected');
          } else if (pc.connectionState === 'failed') {
            console.error('❌ [WEBRTC CONNECTION] Connection failed');
            setCallStatus('idle');
            toast.error('Connection failed');
        ...
        console.log('✅ [WEBRTC] Peer connection created and ready');
        console.log('[WEBRTC] =============================================');

        // Impolite peer creates initial offer
        if (!isPolite) {
          console.log('📤 [WEBRTC] I am impolite, creating initial offer...');
          // Trigger negotiationneeded
          await pc.setLocalDescription();
          await sendSignal('offer', { offer: pc.localDescription });
        } else {
          console.log('📥 [WEBRTC] I am polite, waiting for offer...');
        }

      } catch (error: any) {
        console.error('❌ [WEBRTC] Error creating pc:', error);
  ...
  // Camera control function - only closes PC on unmount or match end
  const stopCamera = (reason: string) => {
    console.log('[CAMERA] stopCamera called. Reason:', reason);

    // Stop local tracks
    if (localStream) {
      console.log('[CAMERA] Stopping local stream tracks');
      localStream.getTracks().forEach((track) => {
        console.log('[CAMERA] Stopping track:', track.kind, track.id);
        track.stop();
      });
      setLocalStream(null);
    }

    // Use replaceTrack to remove video instead of closing PC
    if (videoSenderRef.current && peerConnectionRef.current) {
      console.log('[CAMERA] Using replaceTrack(null) to stop sending video');
      videoSenderRef.current.replaceTrack(null).catch(err => {
        console.error('[CAMERA] Error replacing track with null:', err);
      });
    }
    ...
    // Only close PC for these specific reasons
    const shouldClosePc = reason.includes('unmount') || reason.includes('match ended');
    if (shouldClosePc && peerConnectionRef.current) {
      console.log('[CAMERA] Closing peer connection for:', reason);
  ...
  useEffect(() => {
    return () => {
      console.log('[CAMERA CLEANUP] Component unmounting');
      stopCamera('component unmount');
    };
  }, []);

  // Diagnostic: Log WebRTC state periodically (every 5 seconds)
  useEffect(() => {
    const interval = setInterval(() => {
      if (peerConnectionRef.current || localStream || remoteStream) {
        console.log('📊 [DIAGNOSTICS] ========== WebRTC State ==========');
        console.log('📊 [DIAGNOSTICS] Peer Connection:', {
          exists: !!peerConnectionRef.current,
          connectionState: peerConnectionRef.current?.connectionState,
          iceConnectionState: peerConnectionRef.current?.iceConnectionState,
          signalingState: peerConnectionRef.current?.signalingState,
        });
        console.log('📊 [DIAGNOSTICS] Local Stream:', {
          exists: !!localStream,
        ...
        console.log('📊 [DIAGNOSTICS] Remote Stream:', {
          exists: !!remoteStream,
          hasVideoTrack: !!remoteStream?.getVideoTracks().length,
        ...
        console.log('📊 [DIAGNOSTICS] UI State:', {
          callStatus,
          isCameraOn,
          isMyTurn: room?.current_turn === currentUserId
        });
  ...
  }, [peerConnectionRef.current, localStream, remoteStream, callStatus, isCameraOn, room?.current_turn, currentUserId]);
  ...
  // Fetch ICE servers once per match and cache them
  useEffect(() => {
  ...
  const startCamera = async () => {
    if (!room || !currentUserId) {
      console.log('[CAMERA] Cannot start camera - missing room or userId');
      toast.error('Match not initialized');
      return;
    }

    console.log('📹 [CAMERA] Starting camera for user:', currentUserId);
    setCameraError(null);

    try {
      console.log('📹 [CAMERA] Requesting media devices (video only)');
      ...
      console.log('✅ [CAMERA] Media granted, tracks:', stream.getTracks().map(t => t.kind));
      ...
      // Use replaceTrack to add video to existing connection
      // Transceiver is already sendrecv, so replaceTrack will trigger renegotiation automatically
      if (videoSenderRef.current) {
        const videoTrack = stream.getVideoTracks()[0];
        if (videoTrack) {
          console.log('📹 [CAMERA] Using replaceTrack to add video track');
          await videoSenderRef.current.replaceTrack(videoTrack);
          console.log('✅ [CAMERA] Video track added, negotiation will happen automatically');
        }
      } else {
        console.warn('⚠️ [CAMERA] No video sender ref, PC not ready yet');
      }

      // Send camera state to opponent
      await sendSignal('state', { cameraOn: true });
  ...
  const handleOffer = async (offer: RTCSessionDescriptionInit) => {
    ...
    if (!pc) {
      console.error('❌ [OFFER] No peer connection exists, triggering setup first');
      // PC should be created by now, but if not this is an error
      setCallStatus('idle');
      return;
    }
    ...
    try {
      ...
      console.log('🔍 [OFFER] Collision check:', {
        offerCollision,
      ...
      if (ignoreOfferRef.current) {
        console.log('⛔ [OFFER] Ignoring offer (impolite peer in collision)');
        return;
      }
      ...
      if (pendingIceCandidatesRef.current.length > 0) {
        console.log('🧊 [OFFER] Processing', pendingIceCandidatesRef.current.length, 'pending ICE candidates');
        for (const candidate of pendingIceCandidatesRef.current) {
          try {
            await pc.addIceCandidate(new RTCIceCandidate(candidate));
          } catch (error) {
            console.error('❌ [OFFER] Error adding pending ICE candidate:', error);
          }
        }
        pendingIceCandidatesRef.current = [];
      }
      ...
      console.log('✅ [OFFER] Offer handled successfully');
  ...
  const handleAnswer = async (answer: RTCSessionDescriptionInit) => {
    ...
    const pc = peerConnectionRef.current;
    if (!pc) {
      console.error('❌ [ANSWER] No peer connection exists');
      return;
    }
    ...
    try {
      ...
      console.log('✅ [ANSWER] Answer applied successfully');

      // Process any pending ICE candidates
      if (pendingIceCandidatesRef.current.length > 0) {
        console.log('🧊 [ANSWER] Processing', pendingIceCandidatesRef.current.length, 'pending ICE candidates');
        for (const candidate of pendingIceCandidatesRef.current) {
          try {
            await pc.addIceCandidate(new RTCIceCandidate(candidate));
          } catch (error) {
            console.error('❌ [ANSWER] Error adding pending ICE candidate:', error);
          }
        }
        pendingIceCandidatesRef.current = [];
      }
  ...
  const handleIceCandidate = async (candidate: RTCIceCandidateInit) => {
    ...
    if (!peerConnectionRef.current) {
      console.warn('⚠️ [WEBRTC ICE] No PC yet, queueing ICE candidate');
      console.log('   Queue size:', pendingIceCandidatesRef.current.length + 1);
    ...
    if (!peerConnectionRef.current.remoteDescription) {
      console.warn('⚠️ [WEBRTC ICE] Remote description not set yet, queueing ICE candidate');
      console.log('   Queue size:', pendingIceCandidatesRef.current.length + 1);
      pendingIceCandidatesRef.current.push(candidate);
      return;
    }
  ...
  // Toggle camera - only stops/starts local video, does NOT close peer connection
  const toggleCamera = async () => {
    if (isCameraOn) {
      console.log('📹 [CAMERA] User toggling camera OFF');
      // Stop local tracks
      if (localStream) {
        localStream.getTracks().forEach((track) => {
          track.stop();
      ...
      // Use replaceTrack(null) to stop sending video (keeps transceiver alive)
      if (videoSenderRef.current) {
        console.log('📹 [CAMERA] Using replaceTrack(null) to stop sending video');
      ...
      // Send camera state signal to opponent
      await sendSignal('state', { cameraOn: false });
      toast.success('Camera turned off');
    } else {
      console.log('📹 [CAMERA] User toggling camera ON');
      await startCamera();
      // startCamera already sends the state signal
    }
  };

  // Only call this on true match end events (forfeit, leave, unmount)
  const hangUp = async (reason: string) => {
    console.log('[HANGUP] ===== HANGUP CALLED =====');
    console.log('[HANGUP] Reason:', reason);
    stopCamera(`user initiated: ${reason}`);
    await sendSignal('hangup', { reason });
  };

  // Debug: Reset call state completely
  const resetCall = () => {
    console.log('🔄 [DEBUG RESET] ========== RESETTING CALL STATE ==========');

    // Log current state before reset
    console.log('🔄 [DEBUG RESET] Current state:', {
    ...
    if (localStream) {
      localStream.getTracks().forEach(track => {
        console.log('🔄 [DEBUG RESET] Stopping local track:', track.kind);
        track.stop();
      });
      setLocalStream(null);
    }
    ...
    if (peerConnectionRef.current) {
      console.log('🔄 [DEBUG RESET] Closing peer connection');
      peerConnectionRef.current.close();
      peerConnectionRef.current = null;
    }
    ...
    console.log('✅ [DEBUG RESET] Call state reset complete');
    console.log('🔄 [DEBUG RESET] =======================================');
    toast.success('Call reset - reconnecting will happen automatically');
  };
