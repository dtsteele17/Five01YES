...
export default function LocalMatchPage() {
  ...
  // Read config from URL query params (bolt.new style) or fallback to localStorage
  const p1FromUrl = searchParams?.get('p1');
  const p2FromUrl = searchParams?.get('p2');
  const modeFromUrl = searchParams?.get('mode');
  const formatFromUrl = searchParams?.get('format');
  const doubleOutFromUrl = searchParams?.get('doubleOut');
  const matchId = 'local-match'; // Local matches don't need a real ID
  ...
  const typeScoreInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    try {
      // Try URL query params first (bolt.new style), then localStorage
      if (p1FromUrl && modeFromUrl) {
        const bestOf = parseInt(formatFromUrl || '3');
        const mode = modeFromUrl || '501';
        const doubleOut = doubleOutFromUrl !== 'false';
        ...
        const finalConfig = {
          player1Name: p1FromUrl || 'Player 1',
          player2Name: p2FromUrl || 'Player 2',
          opponentName: p2FromUrl || 'Player 2',
          gameMode: normalized.mode,
          matchFormat: `best-of-${normalized.bestOf}`,
        ...
        setPlayer1Name(finalConfig.player1Name);
        setPlayer2Name(finalConfig.player2Name);
      } else {
  ...
  }, [matchId, router, p1FromUrl, p2FromUrl, modeFromUrl, formatFromUrl, doubleOutFromUrl]);

  useEffect(() => {
    const timer = setTimeout(() => {
      typeScoreInputRef.current?.focus();
    }, 50);
    return () => clearTimeout(timer);
  }, [currentPlayer, currentLeg.legNumber, matchConfig]);
  ...
  const currentScore = currentPlayer === 'player1' ? player1Score : player2Score;
  const player1CheckoutOptions = player1Score > 1 && player1Score <= 170
    ? getCheckoutOptions(player1Score, matchConfig?.doubleOut ?? true)
    : [];
  const player2CheckoutOptions = player2Score > 1 && player2Score <= 170
    ? getCheckoutOptions(player2Score, matchConfig?.doubleOut ?? true)
    : [];
  const isPlayer1OnCheckout = player1Score > 1 && player1Score <= 170;
  const isPlayer2OnCheckout = player2Score > 1 && player2Score <= 170;
