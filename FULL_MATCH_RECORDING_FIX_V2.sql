-- ============================================
-- FIVE01 Darts - FULL MATCH RECORDING FIX (V2)
-- Handles existing views - Run this in Supabase SQL Editor
-- ============================================
...
-- Check if matches is a view first
DO $$
BEGIN
...
-- ============================================
-- 4. Function to update player streaks after match
-- ============================================
...
-- ============================================
-- 5. Function to record match and update stats
-- ============================================
...
BEGIN
  ...
  INSERT INTO matches (
    ...
    'You', p_opponent_name,
    COALESCE((p_user_stats->>'legsWon')::INTEGER, CASE WHEN v_is_win THEN 1 ELSE 0 END),
    COALESCE((p_opponent_stats->>'legsWon')::INTEGER, CASE WHEN v_is_win THEN 0 ELSE 1 END),
  ...
  INSERT INTO match_history (
    ...
    v_match_id::TEXT, p_user_id, COALESCE(p_opponent_id::TEXT, CASE WHEN p_opponent_is_bot THEN 'dartbot' ELSE 'unknown' END), 
    (p_game_mode)::INTEGER, 
  ...
  SELECT
    ...
    COALESCE(us.total_180s, 0) + COALESCE((p_user_stats->>'count180')::INTEGER, 0),
    COALESCE(us.total_checkout_attempts, 0) + COALESCE((p_user_stats->>'checkoutDartsAttempted')::INTEGER, 0),
    COALESCE(us.total_checkouts_made, 0) + COALESCE((p_user_stats->>'checkoutsMade')::INTEGER, 0),
    GREATEST(COALESCE(us.highest_checkout, 0), COALESCE((p_user_stats->>'highestCheckout')::INTEGER, 0)),
    GREATEST(COALESCE(us.best_average, 0), COALESCE((p_user_stats->>'threeDartAvg')::NUMERIC, 0)),
    GREATEST(COALESCE(us.best_first9_average, 0), COALESCE((p_user_stats->>'first9Avg')::NUMERIC, 0)),
    COALESCE(us.total_100_plus, 0) + COALESCE((p_user_stats->>'count100Plus')::INTEGER, 0),
    COALESCE(us.total_140_plus, 0) + COALESCE((p_user_stats->>'count140Plus')::INTEGER, 0),
  ...
  DO UPDATE SET
    ...
    total_180s = user_stats.total_180s + COALESCE((p_user_stats->>'count180')::INTEGER, 0),
    total_checkout_attempts = user_stats.total_checkout_attempts + COALESCE((p_user_stats->>'checkoutDartsAttempted')::INTEGER, 0),
    total_checkouts_made = user_stats.total_checkouts_made + COALESCE((p_user_stats->>'checkoutsMade')::INTEGER, 0),
    highest_checkout = GREATEST(user_stats.highest_checkout, COALESCE((p_user_stats->>'highestCheckout')::INTEGER, 0)),
    best_average = GREATEST(user_stats.best_average, COALESCE((p_user_stats->>'threeDartAvg')::NUMERIC, 0)),
    best_first9_average = GREATEST(user_stats.best_first9_average, COALESCE((p_user_stats->>'first9Avg')::NUMERIC, 0)),
    total_100_plus = user_stats.total_100_plus + COALESCE((p_user_stats->>'count100Plus')::INTEGER, 0),
    total_140_plus = user_stats.total_140_plus + COALESCE((p_user_stats->>'count140Plus')::INTEGER, 0),
  ...
  PERFORM update_player_streaks(p_user_id, CASE WHEN v_is_win THEN 'win' ELSE 'loss' END);
...
-- ============================================
-- 6. Function to get recent matches with all details
-- ============================================
...
BEGIN
  ...
  SELECT
    ...
    CASE 
      ...
      WHEN mh.opponent_id = 'training' THEN 'Training'
  ...
  LEFT JOIN users u ON u.id = mh.opponent_id::uuid
...
-- ============================================
-- 7. Function to get dashboard stats
-- ============================================
...
-- DONE!
-- ============================================
SELECT 'Full match recording fix V2 applied successfully!' as status;
