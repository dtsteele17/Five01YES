...
const GAMEON_PLAYED_KEY_PREFIX = 'training_gameon_played_';
...
// Game On sound functions
export function playGameOnSfx(): void {
  try {
    // Check if sound is enabled in settings (default: true)
    const soundEnabled = localStorage.getItem(GAMEON_ENABLED_KEY);
    if (soundEnabled === 'false') {
      console.log('[GAMEON_SFX] Sound disabled in settings');
      return;
    }

    // Create audio instance if it doesn't exist
    if (!gameOnAudio) {
      gameOnAudio = new Audio(GAMEON_SOUND_URL);
      gameOnAudio.volume = 0.7;
      console.log('[GAMEON_SFX] Audio instance created');
    }

    // Reset to start and play
    gameOnAudio.currentTime = 0;
    gameOnAudio.play().catch((error) => {
      console.error('[GAMEON_SFX] Failed to play game on sound:', error);
    });

    console.log('[GAMEON_SFX] Playing game on sound');
  } catch (error) {
    console.error('[GAMEON_SFX] Error in playGameOnSfx:', error);
  }
}

export function hasPlayedGameOnForSession(sessionId: string): boolean {
  try {
    const key = `${GAMEON_PLAYED_KEY_PREFIX}${sessionId}`;
    return sessionStorage.getItem(key) === 'true';
  } catch (error) {
    console.error('[GAMEON_SFX] Error checking if game on was played:', error);
    return false;
  }
}

export function markGameOnPlayedForSession(sessionId: string): void {
  try {
    const key = `${GAMEON_PLAYED_KEY_PREFIX}${sessionId}`;
    sessionStorage.setItem(key, 'true');
    console.log('[GAMEON_SFX] Marked game on as played for session:', sessionId);
  } catch (error) {
    console.error('[GAMEON_SFX] Error marking game on as played:', error);
  }
}

export function isGameOnSoundEnabled(): boolean {
  try {
    const soundEnabled = localStorage.getItem(GAMEON_ENABLED_KEY);
    // Default to true if not set
    return soundEnabled !== 'false';
  } catch (error) {
    console.error('[GAMEON_SFX] Error checking game on sound enabled:', error);
    return true;
  }
}

export function setGameOnSoundEnabled(enabled: boolean): void {
  try {
    localStorage.setItem(GAMEON_ENABLED_KEY, enabled ? 'true' : 'false');
    console.log('[GAMEON_SFX] Game on sound enabled set to:', enabled);
  } catch (error) {
    console.error('[GAMEON_SFX] Error setting game on sound enabled:', error);
  }
}
