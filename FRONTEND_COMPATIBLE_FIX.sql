-- FRONTEND COMPATIBLE FIX: Match exact return structure the code expects
-- This will make the dartbot game launch properly
...
BEGIN
  ...
  -- Return in the EXACT format the frontend fixtures code expects
  RETURN json_build_object(
    'success', true,
    'match_id', v_match_id,  -- ✅ Frontend expects: matchData.match_id
    'event', json_build_object(
      'id', v_event.id,       -- ✅ Frontend expects: matchData.event?.id
      'name', COALESCE(v_event.event_name, 'League Match'), -- ✅ Frontend expects: matchData.event?.name  
      'format_legs', COALESCE(v_event.format_legs, 3)       -- ✅ Frontend expects: matchData.event?.format_legs
    ),
    'opponent', json_build_object(
      'id', v_opponent.id,    -- ✅ Frontend expects: matchData.opponent?.id
      'name', COALESCE(v_opponent.first_name || ' ' || v_opponent.last_name, 'Unknown Opponent') -- ✅ Frontend expects: matchData.opponent?.name
    )
  );
